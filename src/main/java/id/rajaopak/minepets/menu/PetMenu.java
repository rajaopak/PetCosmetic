package id.rajaopak.minepets.menu;
...
public class PetMenu {
    ...
    public void openMenu(int page) {
        ...
        List<PetType<?>> list = MinePets.getInstance().getPetManager().getPetList();
        int maxPerPage = 28;
        int size = list.size();
        ...
        finalList.forEach(petType -> {
            ...
            if (MinePets.getInstance().getPetManager().hasPet(player)) {
                if (MinePets.getInstance().getPetManager().getPet(player).getClass().equals(petType.getPet())) {
                    item.setGlowing();
                }
            }
        ...
        gui.setItem(52, ItemBuilder.from(Material.ARMOR_STAND).setName("&eSet Pet to Adult").setFlags(ItemFlag.HIDE_ATTRIBUTES).build(), inventoryClickEvent -> {
            if (MinePets.getInstance().getPetManager().hasPet(player)) {
                gui.close(player);
                if (!MinePets.getInstance().getPetManager().getPet(player).isBaby()) {
                    ChatUtil.sendMessage(player, "&cYour pet is already Adult!");
                    return;
                }

                MinePets.getInstance().getPetManager().getPet(player).setBaby(false);
            } else {
                ChatUtil.sendMessage(player, "&cYou don't have any selected pet!");
            }
        });
        ...
        gui.setItem(53, ItemBuilder.from(Material.TOTEM_OF_UNDYING).setName("&eSet Pet to Baby").setFlags(ItemFlag.HIDE_ATTRIBUTES).build(), inventoryClickEvent -> {
            if (MinePets.getInstance().getPetManager().hasPet(player)) {
                ...
                MinePets.getInstance().getPetManager().getPet(player).setBaby(true);
            } else {
                ChatUtil.sendMessage(player, "&cYou don't have any selected pet!");
            }
        });
